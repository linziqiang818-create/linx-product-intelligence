...
const products = (await readFile(join(publicRoot, "real-products.json"), "utf8")).replace(/<\/script/gi, "<\\/script");
const rejected = (await readFile(join(publicRoot, "rejected-products.json"), "utf8")).replace(/<\/script/gi, "<\\/script");
const productData = `<script id="linx-builtin-data">window.__LINX_DATA__={products:${products},trash:${rejected}};</script>`;
...
html = html
  ...
  .replace("</body>", () => `${productData}${diagnostics}<script id="linx-app-bundle">${javascript}</script></body>`);
...
if (rootPosition < 0 || dataPosition < rootPosition || appPosition < dataPosition) {
