...
function ResearchCard({ product, decision, onDecision, compact = false }: { product: BatchCalibrationCandidate; decision?: ResearchDecision; onDecision: (decision: ResearchDecision) => void; compact?: boolean }) {
...
export default function BatchCalibrationPanel({ products }: { products: BatchCalibrationCandidate[] }) {
  ...
  return <section className="batch-calibration preference-lab">
    <div className="panel batch-head">
      <div><span className="eyebrow">LINX PREFERENCE LAB</span><h2>候选研究与新品挑战</h2><p>不再判断抽象名次。先回答“是否值得花 30 分钟研究”，再用具体产品之间的选择逐渐建立你的真实排序标准。</p></div>
      <div className="calibration-status"><b>{coverage.decidedGroups}/{groups.length}</b><span>产品组已判断</span><i>{candidateReviewed}/20 候选已确认</i></div>
    </div>

    <div className="batch-kpis">
      <div><b>{coverage.splitProducts}</b><span>差异组产品优先探索</span></div><div><b>{candidateReviewed}/20</b><span>候选研究池已判断</span></div><div><b>{explorationReviewed}/{challengeUniverse.length}</b><span>探索样品已看</span></div><div><b>{comparisonCount}</b><span>具体偏好选择</span></div>
    </div>

    <section className="panel batch-section">
      <div className="section-title"><div><span className="eyebrow">STEP 1 · RESEARCH POOL</span><h2>候选研究池</h2><p>这 20 个产品都可以保留。只有真正愿意继续花时间核算和研究的，才选择“值得花 30 分钟研究”。</p></div><button onClick={holdRemainingCandidates}>其余全部先保留</button></div>
      <div className="research-list">{candidatePool.map((product) => <ResearchCard key={product.asin} product={product} decision={state.research[product.asin]?.decision} onDecision={(decision) => setResearch(product.asin, decision, "candidate")} />)}</div>
    </section>
    ...
    <section className="panel batch-section">
      ...
      {challengeBatch.length > 0 ? <div className="exploration-grid">{challengeBatch.map((product) => <ResearchCard compact key={product.asin} product={product} decision={state.research[product.asin]?.decision} onDecision={(decision) => setResearch(product.asin, decision, "challenge")} />)}</div> : <div className="empty"><b>当前产品已经全部看过</b><span>下一步可导入新的市场样品，继续挑战现有候选池。</span></div>}
    </section>
    ...
    <section className="panel batch-section">
      ...
      {challengePairs.length > 0 ? <div className="duel-list">{challengePairs.map((pair) => <article className="duel" key={pair.id}>
        <div className="duel-product"><ProductImage product={pair.challenger} /><b>{pair.challenger.titleZh}</b><small>新品挑战者 · {pair.challenger.asin}</small></div>
