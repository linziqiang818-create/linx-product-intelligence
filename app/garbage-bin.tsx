...
export default function GarbageBin({ products, restore, empty }: {
  products: GarbageProduct[];
  restore: (product: GarbageProduct) => void;
  empty: () => void;
}) {
  ...
  return <section className="panel rejected-panel" ref={anchor}>
    <div className="section-title">
      ...
      <div className="actions"><span className="tag">{products.length} 款硬性淘汰</span><button className="danger" disabled={!products.length} onClick={empty}>彻底清空</button></div>
    </div>
    ...
    <div className="table-wrap">
      <table>
        ...
        <tbody>{pager.pageItems.map(product => <tr key={product.asin}>
          ...
          <td><button className="link" onClick={() => restore(product)}>恢复并编辑</button></td>
        </tr>)}</tbody>
      </table>
